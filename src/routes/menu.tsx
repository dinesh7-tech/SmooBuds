  ...
  return (
    <div className="min-h-screen bg-cream text-ink font-body pb-28">
      ...
      <main className="mx-auto max-w-5xl px-4 sm:px-6 mt-6 md:mt-8">
        ...
        {/* Live Session Order History */}
        {isVerified && sessionOrders.length > 0 && (
          <section className="mt-16 border-t border-sage/10 pt-10 pb-4">
            <h2 className="font-display text-2xl font-extrabold text-sage-deep mb-6 flex items-center gap-2">
              <Clock size={24} className="text-gold" /> Your Session Orders
            </h2>
            <div className="grid gap-4 sm:gap-6 sm:grid-cols-2">
              {sessionOrders.map((ord, idx) => (
                ...
                >
                  ...
                  <ul className="space-y-3 mb-5 text-sm text-sage-deep">
                    {ord.order_items.map((item) => (
                      <li key={item.id} className="flex flex-col bg-white/40 p-3 rounded-xl border border-sage/5">
                        <div className="flex justify-between items-center">
                          <span className="font-bold">{item.quantity} × {item.item_name}</span>
                          <span className="text-[0.8rem] font-bold text-sage">₹{item.item_price * item.quantity}</span>
                        </div>
                        {item.notes && (
                          <span className="text-[0.7rem] text-gold-soft font-medium bg-gold/10 rounded-md px-2.5 py-1 mt-2 w-fit flex items-center gap-1.5">
                  ...
                  <div className="flex justify-between items-center border-t border-sage/10 pt-4 font-display text-lg font-extrabold text-sage-deep">
                    <span>Total Amount</span>
                    <span className="text-gold-gradient">₹{ord.total_amount}</span>
                  </div>
                </div>
              ))}
            </div>
